...
public class EdadExacta {
	...
	public static void main(String[] args) {
		try (Scanner s = new Scanner(System.in)) {
			System.out.println(
				"Indica tu fecha de nacimiento (" + PATRON + ")");
			...
			try {
				...
				System.out.println(
					String.format("Edad: %d años, %d meses y %d días",
				...
				if (edad.getMonths() == 0) {
					if (edad.getDays() == 0) {
						...
						System.out.println(
							"¡Felicidades! Tu cumpleaños ha sido hace poco...");						
					}
				} else if (edad.getMonths() == 11) {
					System.out.println(
						"¡Felicidades! Tu cumpleaños se acerca...");						
				}
			} catch (DateTimeParseException e) {
				System.err.println(String.format(
					"La fecha indicada ('%s') " 
					+ "no sigue el patrón esperado ('%s').", 
						fechaLeida, PATRON)); 
			}
		}
	}
